...
def calcu_idf(tokenizer_path):
    ...
    output_dir = "eubhd_idf"
    os.makedirs(output_dir, exist_ok=True)
    output_path = os.path.join(output_dir, f"token_idf_{tokenizer_path.split('/')[-1]}.pkl")
    pickle.dump(np.array([math.log(total_documents / (document_frequency[i] + 1)) for i in range(len(tokenizer.vocab))]), open(output_path, "wb"))
